...
public sealed class LinqExercises
{
    ...
    {
       return  UniversityData.Students.OrderBy(s => s.LastName).ThenBy(s => s.FirstName).Select((st => $"{st.IndexNumber},{st.FirstName},{st.LastName}")).ToList();
    }
    ...
    public IEnumerable<string> Task04_FirstAnalyticsCourse()
    {
        return [UniversityData.Courses.Where(c => c.Category == "Analytics")
            .Select(p => $"{p.Title} {p.StartDate}").FirstOrDefault() ?? "Analytics course not found"];
    }
    ...
    public IEnumerable<string> Task08_DistinctStudentCities()
    {
        return UniversityData.Students.OrderBy(s => s.City)
            .Select(s => s.City).Distinct().ToList();
    }
                ...
                (l, courses) => new
                {
                    ...
                    Grades = courses
                        .SelectMany(c => UniversityData.Enrollments
                            .Where(e => e.CourseId == c.Id && e.FinalGrade.HasValue)
                            .Select(e => e.FinalGrade.Value))
                })
            .Select(l => $"{l.FirstName} {l.LastName} {(l.Grades.Any() ? l.Grades.Average() : 0):F2}")
            .ToList();
    }
